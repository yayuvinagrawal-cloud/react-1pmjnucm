...
function Hero({ setTab }) { const particles = Array.from({ length: 22 }, (_, i) => i); const sTier = ALL_KITS.filter((kit) => kit.tier === "S").slice(0, 7); return <section className="hero"><div className="heroGridOverlay" /><div className="heroGlow" /><div className="particles">{particles.map((item) => <span key={item} style={{ "--i": item, "--x": ((item * 47) % 100) + "%", "--y": (10 + ((item * 29) % 76)) + "%" }} />)}</div><div className="heroInner"><div className="heroKicker">Season 16 Competitive Index</div><div className="heroCredit">Made by justcyril</div><h1><span>DOMINATE 5v5.</span><span>BUILD <em>SMARTER.</em></span></h1><p>The only BedWars meta guide you need. Builds, drafts, kits, and strategy for serious players.</p><div className="heroActions"><RippleButton className="btn primary" onClick={() => setTab("meta")}>View Meta Builds</RippleButton><RippleButton className="btn outline" onClick={() => setTab("draft")}>Build Your Draft</RippleButton></div><div className="heroFeatured"><span className="heroFeaturedLabel">S-tier watchlist</span><div>{sTier.map((kit) => <button key={kit.name} onClick={() => setTab("kits")}>{kit.name}</button>)}</div></div><div className="heroStats"><div><strong><CountUp value={ALL_KITS.length} /></strong><span>Kits tracked</span></div><div><strong><CountUp value={META_BUILDS.length} /></strong><span>Meta builds</span></div><div><strong><CountUp value={ROLE_GUIDE.length} /></strong><span>Roles covered</span></div><div><strong><CountUp value={16} /></strong><span>Season</span></div></div></div></section>; }
